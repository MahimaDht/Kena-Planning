                                        ...
                                        <div class="row">
                                         ...
                                         <div class="col-md-6">
                                            ...
                                            <div class=" col-md-6">
                                                ...
                                                <select class="form-control select2" name="process[]" id="process" required multiple >
                                                  
                                              </select>
                                          </div>
                                            <div class="col-md-6">
                                                <label class="form-label">Machine*</label>
                                               <select class="form-control select2" name="machine[]" id="machine" multiple>
                                                <option value="">Select</option>
                                                <?php 
                                                 $machineArray = explode(',', $editdata->machine_id);
                                                foreach ($machines as $key => $value) { ?>
                                                    
                                                    <option value="<?php echo $value->id ?>" <?php if(isset($editdata) && in_array($value->id, $machineArray)){echo "selected";}?>><?php echo $value->machine_name ;?></option>
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $("#process").html('<option value="">Select</option>'+response.process);
              if (selectedprocess) {
                // if passed as comma-separated string → convert to array
                if (typeof selectedprocess === "string") {
                    selectedprocess = selectedprocess.split(",").map(function(item) {
                        return item.trim();
                    });
                }

                // assign array to multi-select
                $("#process").val(selectedprocess).trigger("change");
            }
        },
