                        ...
                        <div class="col-11 mx-auto">
                                                        ...
                                                        foreach ($layoutGroups as $groupName => $items):
                                                           ...
                                                           // print_r($firstItem);
                                                            ...
                                                            <div class="group-card" id="parameter_box_<?= $layoutIndex ?>_<?= $groupIndex ?>">
                                                               ...
                                                               <?php
                                                                $selectedList = (array) ($selectedProcesses[$firstItem->product_layout_id][$firstItem->process_group_id] ?? []);

                                                                foreach ($items as $proc):

                                                                    // show only selected processes
                                                                    if (!in_array($proc->process_id, $selectedList)) continue;
                                                                ?>
                                                                    <option 
                                                                        value="<?= $proc->process_id ?>"
                                                                        data-process_code="<?= $proc->process_code ?>"
                                                                        data-processmappingLayoutId="<?= $proc->processmappingLayoutId ?>"
                                                                        selected
                                                                    >
                            ...
                            <!-------------- process sequence ------------->
                                ...
                                <!-----------end-------------------->
                                    ...
                                    <button type="submit"   onclick="return formValidate()"  class="btn btn-primary mt-4">Save changes</button>
                                </form>
                            <?php }else{ ?>
                                <form class="form-group" action="<?= base_url('Transaction/save_salesProcess') ?>" method="post">
                                    ...
                                    <div class="row bg-light p-3 mt-4 mb-3 rounded mx-2">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <input type="hidden" class="form-control" 
                                                   name="routing_id" id="routing_id"  value='<?php if(isset($routbycode)){ echo $routbycode->id ;}?>' required>
                                            <input type="hidden" class="form-control" 
                                ...
                                <div class="col-md-12">
                                    ...
                                    <button type="submit"  onclick="return formValidate()" class="btn btn-primary">Save changes</button>
...
function submitRout(routingId=null)
{
     ...
     var product_type_id=$("#product_id").val();
             ...
             //allowedProcessesByRouting = response;
              ...
              routingSequence = response; 
                ...
                $('.process-select').each(function() {
                    ...
                    selectBox.empty(); // Clear
                    selectBox.append('<option value="">Select Process</option>');

                    if (response[layoutId] && response[layoutId][groupId]) {

                        // Add options
                        let options = response[layoutId][groupId]['options'];
                        options.forEach(function(opt){
                            selectBox.append('<option value="'+opt.id+'" data-process_code="'+opt.processCode+'" data-processmappinglayoutid="'+opt.processmappingLayoutId+'">'+opt.name+'('+opt.processCode+')</option>');
                        });

                        // Set selected process
                        selectBox.val(response[layoutId][groupId]['processes']).trigger('change');
                    }
                });
    ...
    $(document).ready(function() {
        ...
        var routID='<?php if(isset($routbycode)){echo $routbycode->id ;}?>';
        if(routID!='')
        {
            submitRout(routID);
        }
